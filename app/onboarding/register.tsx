...
export default function RegisterScreen() {
  ...
  function validate() {
    ...
    if (username.length < 3) {
      errs.username = 'Nick musi mieć co najmniej 3 znaki.';
      valid = false;
    } else if (username.length > 16) {
      errs.username = 'Nick nie może mieć więcej niż 16 znaków.';
      valid = false;
    }
    // Email: prosty regex
    if (!/^\S+@\S+\.\S+$/.test(email)) {
    ...
    if (password.length < 8) {
      ...
      errs.password = 'Hasło musi zawierać literę i cyfrę.';
      valid = false;
    }
    setErrors(errs);
    return valid;
  ...
  async function handleRegister() {
    ...
    if (success) {
      router.replace('/onboarding/infoSlide');
    }
  }

  useEffect(() => {
    if (error && !errors.username && !errors.email && !errors.password) {
      Alert.alert(
        'Coś poszło nie tak',
    ...
    <LinearGradient colors={['#000', '#1A0A1A', '#200720ff', '#FF1744']} style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <View style={styles.inner}>
          ...
          <Text style={styles.subtitle}>Załóż konto, aby zacząć korzystać z aplikacji</Text>
          ...
          <TouchableOpacity style={[styles.createButton, loading && styles.createButtonLoading]} onPress={handleRegister} disabled={loading}>
            <Text style={styles.createButtonText}>{loading ? 'Zakładanie...' : 'Załóż konto'}</Text>
          </TouchableOpacity>
          ...
          <Text style={styles.loginText}>
            Masz już konto?{' '}
            <Text style={styles.loginLink} onPress={() => router.push('/onboarding/login')}>
              Zaloguj się
            </Text>
          </Text>
        </View>
        <Text style={styles.termsText}>
          Zakładając konto, akceptujesz <Text style={styles.termsLink}>Politykę prywatności</Text> i <Text style={styles.termsLink}>Regulamin</Text>.
        </Text>
      </ScrollView>
    </LinearGradient>
  );
